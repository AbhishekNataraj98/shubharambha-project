...
const recordSchema = z.object({
  ...
  category: z.enum(['labour', 'material', 'contractor_fee', 'other']),
  paid_to: z.string().trim().min(2),
...
export async function POST(
  ...
  const paidToUserId = uuidRegex.test(payload.paid_to)
    ? payload.paid_to
    : project.contractor_id ?? user.id
  const safeDescription = payload.description?.trim() || 'Payment recorded'
  ...
  const primaryInsert = await admin
    ...
    .insert({
      ...
      paid_to_category: payload.category,
  ...
  if (project.contractor_id) {
    await admin.from('notifications').insert({
      ...
      }).format(payload.amount)} for ${payload.category}. Please review and approve.`,
