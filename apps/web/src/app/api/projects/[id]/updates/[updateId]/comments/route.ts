...
export async function POST(
  ...
  const [{ data: project }, { data: actorProfile }, { data: workerMembers }] = await Promise.all([
    ...
    admin.from('project_members').select('user_id,role').eq('project_id', projectId).eq('role', 'worker'),
  ])

  const actorRole = actorProfile?.role ?? ''
  const recipientIds = new Set<string>()
  if (actorRole === 'customer') {
    if (project?.contractor_id) recipientIds.add(project.contractor_id)
    for (const member of workerMembers ?? []) {
      if (member.user_id) recipientIds.add(member.user_id)
    }
  } else if (actorRole === 'contractor' || actorRole === 'worker') {
    if (project?.customer_id) recipientIds.add(project.customer_id)
  }
  ...
  if (recipientIds.size > 0) {
    ...
    const primaryNotification = await admin.from('notifications').insert(rows)
